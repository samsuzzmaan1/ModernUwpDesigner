...
using WinRT;
using XSurfUwp.Common;
using XSurfUwp.Composition;
...
public sealed partial class DT
{
    ...
    public static readonly DependencyProperty InvertColorsProperty = DependencyProperty.RegisterAttached("InvertColors", typeof(bool), typeof(DT), new PropertyMetadata(false, InvertColorsPropertyChangedCallback));
    
    public static readonly DependencyProperty InvertColorsVisualProperty = DependencyProperty.RegisterAttached("InvertColorsVisual", typeof(object), typeof(DT), new PropertyMetadata(null));
    ...
    public static void SetInvertColors(DependencyObject dependencyObject, bool value)
    {
        dependencyObject.SetValue(InvertColorsProperty, value);
    }

    public static bool GetInvertColors(DependencyObject dependencyObject)
    {
        return (bool)dependencyObject.GetValue(InvertColorsProperty);
    }

    public static void SetInvertColorsVisual(DependencyObject dependencyObject, object value)
    {
        dependencyObject.SetValue(InvertColorsVisualProperty, value);
    }

    public static object GetInvertColorsVisual(DependencyObject dependencyObject)
    {
        return dependencyObject.GetValue(InvertColorsVisualProperty);
    }

    [DynamicWindowsRuntimeCast(typeof(UIElement))]
    [DynamicWindowsRuntimeCast(typeof(ContainerVisual))]
    [DynamicWindowsRuntimeCast(typeof(SpriteVisual))]
    [DynamicWindowsRuntimeCast(typeof(Shape))]
    [DynamicWindowsRuntimeCast(typeof(TextBlock))]
    private static void InvertColorsPropertyChangedCallback(DependencyObject d, DependencyPropertyChangedEventArgs e)
    {
        var element = (UIElement)d;
        bool value = (bool)e.NewValue;

        if (Features.IsOnWindows11OrHigher)
        {
            using var factory = ActivationFactory.Get("Windows.UI.Xaml.UIElement");
            if (factory.TryAs<IUIElementStaticsPrivate>(typeof(IUIElementStaticsPrivate).GUID, out var elementStaticsPrivate) >= 0 &&
                elementStaticsPrivate is not null)
            {
                using var staticsPrivate = elementStaticsPrivate;

                Marshal.QueryInterface(((IWinRTObject)element).NativeObject.ThisPtr, new("676d0be9-b65c-41c6-ba40-58cf87f201c1"), out nint ppv);
                var pLayerVisual = elementStaticsPrivate.Vftbl.GetElementLayerVisual(ppv);
                Marshal.Release(ppv);

                if (pLayerVisual is not 0)
                {
                    var layerVisual = LayerVisual.FromAbi(pLayerVisual);
                    Marshal.Release(pLayerVisual);
        ...
        if (value)
        {
            ...
            using var effectFactory = compositor.CreateEffectFactory(effect);
            var effectBrush = effectFactory.CreateBrush();
            effectBrush.SetSourceParameter("source", compositor.CreateBackdropBrush());

            CompositionBrush brush = effectBrush;
            /*if (element is Shape shape)
            {
                var maskBrush = compositor.CreateMaskBrush();
                maskBrush.Mask = shape.GetAlphaMask();
                maskBrush.Source = brush;
                brush = maskBrush;
            }
            else if (element is TextBlock textBlock)
            {
                var maskBrush = compositor.CreateMaskBrush();
                maskBrush.Mask = textBlock.GetAlphaMask();
                maskBrush.Source = brush;
                brush = maskBrush;
            }*/
            ...
            {
                ContainerVisual visual = spriteVisual;
                if (childVisual is not null)
                {
                    visual = compositor.CreateContainerVisual();
                    visual.RelativeSizeAdjustment = Vector2.One;
                    visual.Children.InsertAtTop(childVisual);
                    visual.Children.InsertAtTop(spriteVisual);
                    SetInvertColorsVisual(d, spriteVisual);
                }
        ...
        else
        {
            if (childVisual is ContainerVisual childContainer)
            {
                ...
                else if (GetInvertColorsVisual(d) is SpriteVisual childInvertVisual)
                {
                    childContainer.Children.Remove(childInvertVisual);
                    SetInvertColorsVisual(d, null);
